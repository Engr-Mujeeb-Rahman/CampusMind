...
import { STUDY_PLANNER_DATA } from '../constants/studyPlanner';
    ...
    <DashboardLayout>
      <div className="w-full max-w-[900px] mx-auto">
        <section className="mb-10 text-center md:text-left">
          <h2 className="font-headline-xl text-headline-xl text-on-surface mb-2">{STUDY_PLANNER_DATA.title}</h2>
          <p className="font-body-md text-on-surface-variant max-w-2xl">{STUDY_PLANNER_DATA.description}</p>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-gutter">
          <div className="md:col-span-2 bg-surface-container-lowest rounded-xl border border-outline-variant p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-headline-lg text-on-surface">Weekly Schedule</h3>
              <div className="flex gap-2">
                <button className="p-2 hover:bg-surface-container-low rounded-full transition-colors text-on-surface-variant" aria-label="Previous week">
                  <ChevronLeft className="size-5" aria-hidden="true" />
                </button>
                <button className="p-2 hover:bg-surface-container-low rounded-full transition-colors text-on-surface-variant" aria-label="Next week">
            ...
            <div className="grid grid-cols-7 gap-2 text-center">
              {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d) => (
                <div key={d} className="text-outline font-label-md">{d}</div>
              ))}
            ...
            <div className="mt-6 space-y-3">
              <div className="flex items-center gap-4 p-3 bg-surface-container-low rounded-lg">
                <span className="text-label-md font-bold w-14 shrink-0">09:00</span>
                <div className="h-2 w-2 rounded-full bg-primary shrink-0" />
                <span className="text-body-sm text-on-surface">Quantum Mechanics Review</span>
              </div>
              <div className="flex items-center gap-4 p-3 bg-surface-container-low rounded-lg">
                <span className="text-label-md font-bold w-14 shrink-0">14:00</span>
                <div className="h-2 w-2 rounded-full bg-tertiary shrink-0" />
                <span className="text-body-sm text-on-surface">Problem Set #4</span>
              </div>
            </div>
          </div>

          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-6 flex flex-col">
            <h3 className="font-headline-lg text-on-surface mb-4">Daily Summary</h3>
            <div className="flex-1 space-y-6">
              <div className="text-center py-4 bg-primary-fixed/30 rounded-xl">
                <p className="text-label-md text-primary">Total Study Time</p>
                <p className="text-headline-xl font-bold text-primary">4.5h</p>
              </div>
              ...
              <div className="pt-4 border-t border-outline-variant">
                <p className="font-label-md text-on-surface mb-2">Next Milestone</p>
                <p className="text-body-sm text-on-surface-variant">Chapter 3 Quiz in 2 days</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div
          className={`space-y-6 transition-all duration-500 ${
            showGenerated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {STUDY_PLANNER_DATA.timetable.slice(0, 3).map((item) => (
              <div
                key={item.day}
                className={`${item.span} ${
            ...
            <div className="md:col-span-4 space-y-2">
              {STUDY_PLANNER_DATA.timetable.slice(3).map((item) => (
                <div
                  key={item.day}
